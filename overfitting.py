...
from common import logger, DATADIR, filter_pt, filter_ht, Columns, time_and_log, columns_to_numpy, columns_to_numpy_iter_one_bkg, read_training_features, set_matplotlib_fontsizes, imgcat
...
def main():
    ...
    train_signal_cols = [Columns.load(f) for f in glob.glob(DATADIR+'/train_signal/*350*.npz')]
    test_signal_cols = [Columns.load(f) for f in glob.glob(DATADIR+'/test_signal/*350*.npz')]
    ...
    with time_and_log('Scoring...'):
        X_train, y_train, weight_train = columns_to_numpy_iter_one_bkg(
        ...
        X_test, y_test, weight_test = columns_to_numpy_iter_one_bkg(
    ...
    hist_sig_train, _ = np.histogram(score_train[y_train==1], score_axis, density=True)#, weights=weight_train[y_train==1])
    hist_bkg_train, _ = np.histogram(score_train[y_train==0], score_axis, density=True)#, weights=weight_train[y_train==0])
    hist_sig_test, _ = np.histogram(score_test[y_test==1], score_axis, density=True)#, weights=weight_test[y_test==1])
    hist_bkg_test, _ = np.histogram(score_test[y_test==0], score_axis, density=True)#, weights=weight_test[y_test==0])
    ...
    plt.savefig('overfit.pdf', bbox_inches='tight')
    imgcat('overfit.png')
